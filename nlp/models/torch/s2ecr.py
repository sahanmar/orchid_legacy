...
class S2ECR(BertPreTrainedModel):
    def __init__(
        self,
        embeds_dim: int,
        max_span_length: int,
    ...
    def _calc_mention_logits(self, start_mention_reps, end_mention_reps):
        start_mention_logits = self.mention_start_classifier(start_mention_reps).squeeze(
            -1
        ...
        mention_logits = (
            joint_mention_logits + start_mention_logits.unsqueeze(-1) + end_mention_logits.unsqueeze(-2)
        )
    ...
    def forward(self, encoded_doc, attention_mask, gold_clusters=None, return_all_outputs=False):

        # Compute representations
        start_mention_reps = self.start_mention_mlp(encoded_doc) if self.do_mlps else encoded_doc
        end_mention_reps = self.end_mention_mlp(encoded_doc) if self.do_mlps else encoded_doc

        start_coref_reps = self.start_coref_mlp(encoded_doc) if self.do_mlps else encoded_doc
        end_coref_reps = self.end_coref_mlp(encoded_doc) if self.do_mlps else encoded_doc
